...
int main()
{
	...
	for (int p = 0; p < 4; p++)
	{
		...
		cout << personList[p].name << "\t" << personList[p].home->city <<
			", " << personList[p].home->state << ", " << personList[p].home->zip << endl;
	}
...
void SetupLocations(Location* locationList)
{
	...
	locationList[4].state = "TX";
	locationList[4].zip = 76513;
